...
export function compilePresetChannels(
  ...
  const order = characterId !== -1
    ? resolveOrderIdentifiers(preset, characterId, onlyEnabled)
    : null
  ...
  for (const p of picked) {
    ...
    if (enableTemplateProcessing && content) {
      const templateResult = processLiquidTemplateVariables(content, opt.templateOptions)
...
export function createRoleWithLiquidTemplates(
  ...
  const defaultRole: Partial<Role> = {
    ...
    character_book: role.character_book || { name: '', entries: [], scan_depth: 4, token_budget: 200, recursive_scanning: false, extensions: {} },
