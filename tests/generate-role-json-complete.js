...
// 从源文件复制的完整LiquidJS模板处理逻辑
function createLiquidTemplateProcessor() {
  const { Liquid } = require('liquidjs')

  return class LiquidTemplateProcessor {
    constructor(options = {}) {
      this.liquid = new Liquid({
        strict: options.strict || false,
        ...options.liquidOptions
      })
      this.defaultVariables = options.variables || {}
      this.keepVariableDefinitions = options.keepVariableDefinitions ?? true
      this.maxRecursionDepth = options.maxRecursionDepth || 10
    }

    processTextSync(text, options = {}) {
      const variables = { ...this.defaultVariables, ...(options.variables || {}) }
      let processedText = text

      // 预处理：移除注释，提取变量定义
      const { processed: preprocessed, setVariables, variableDefinitions } = this.preprocessText(processedText)

      // 合并变量
      const allVariables = { ...variables, ...setVariables }

      // 递归处理直到模板稳定
      let previousText = ''
      let depth = 0
      while (previousText !== processedText && depth < this.maxRecursionDepth) {
        previousText = processedText

        try {
          processedText = this.liquid.parseAndRenderSync(processedText, allVariables)
        } catch (error) {
          if (options.strict) {
            throw new Error(`模板渲染失败: ${error.message}`)
          }
          // 非严格模式下，跳过未处理的变量
          processedText = this.removeUnprocessedVariables(processedText)
        }
      ...
      // 后处理：处理变量定义
      processedText = this.postprocessText(processedText, variableDefinitions, !this.keepVariableDefinitions)

      // 分析变量
      const analysisResult = this.analyzeVariables(text, allVariables)

      return {
        processedText,
        variables: analysisResult.all,
        setVariables,
        usedVariables: analysisResult.used,
        unprocessedVariables: analysisResult.unprocessed,
    ...
    preprocessText(text) {
      ...
      // 查找所有 {{setvar::name::value}} 模式
      const setvarRegex = /{{\s*setvar\s*::\s*([^=::\s]+)\s*::\s*(.*?)\s*}}/gs
      let match
      ...
      for (let i = 0; i < lines.length; i++) {
        const line = lines[i]
        const newLine = line.replace(setvarRegex, (fullMatch, varName, varValue) => {
          ...
          setVariables[varName.trim()] = cleanValue
          setvarLines.push(`{% assign ${varName.trim()} = "${cleanValue.replace(/"/g, '\\"')}" %}`)
      ...
      if (setvarLines.length > 0) {
        processedText = setvarLines.join('\n') + '\n' + processedText
      }
    ...
    postprocessText(text, variableDefinitions, removeDefinitions = true) {
      if (!removeDefinitions || variableDefinitions.length === 0) {
        return text
      }
      ...
      const filteredLines = lines.filter(line => {
        ...
        return !trimmed.startsWith('{% assign ') && trimmed.endsWith('%}')
      })

      return filteredLines.join('\n')
    }

    removeUnprocessedVariables(text) {
      // 移除未处理的 {{variable}} 格式
      return text.replace(/{{\s*[^}]+\s*}}/g, '')
    }

    analyzeVariables(text, availableVariables = {}) {
      const allVariables = []
      const usedVariables = []
      ...
      while ((match = variableRegex.exec(text)) !== null) {
        ...
        const variable = {
          name: varName,
          value: availableVariables[varName] || '',
      ...
      return {
        all: allVariables,
        used: [...new Set(usedVariables)],
        unprocessed: unprocessedVariables
      }
    }

    getVariableType(varName) {
      if (varName.startsWith('setvar::')) return 'variable'
      if (varName.startsWith('#') || varName.startsWith('/*')) return 'comment'
      return 'output'
    }
...
async function generateCompleteRoleJSON() {
  try {
    ...
    // 使用完整的LiquidJS模板系统
    const LiquidTemplateProcessor = createLiquidTemplateProcessor()
...
function generateDetailedAnalysisReport(originalRole, processedRole, data) {
...
### 变量定义详情
${data.systemResult.variableDefinitions.map(v => `- **${v.name}**: "${v.value}" (行 ${v.line})`).join('\n')}
