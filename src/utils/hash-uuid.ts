...
export class HashUuidGenerator {
	 ...
	 * 适用于需要考虑文件位置和时间的场景
	 */
	static fromFilePathAndMtime(filePath: string, mtime: number): string {
	...
	private static hashToUuid(hash: string): string {
		...
		// 提取UUID各部分
		const part1 = hash.substring(0, 8)   // 8位
		const part2 = hash.substring(8, 12)  // 4位
		const part3 = hash.substring(12, 16) // 4位
		const part4 = hash.substring(16, 20) // 4位
		const part5 = hash.substring(20, 32) // 12位
