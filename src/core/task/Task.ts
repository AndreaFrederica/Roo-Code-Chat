...
export class Task extends EventEmitter<TaskEvents> implements TaskLike {
	...
	private async saveClineMessages() {
		try {
			...
			const { historyItem, tokenUsage } = await taskMetadata({
				...
				scope: this.rolePromptData?.role?.scope === "global" ? "global" : "workspace", // 根据角色的 scope 设置任务范围
			})
	...
	async ask(
		...
		if (this.lastMessageTs !== askTs) {
			...
			console.log(`[Task] ERROR: ask promise was ignored! lastMessageTs=${this.lastMessageTs} !== askTs=${askTs}`)
	...
	private async resumeTaskFromHistory() {
		...
		if (response === "messageResponse") {
			...
			if (this.userMessageContent.length > 0) {
				console.log(`[Task] Clearing ${this.userMessageContent.length} pending messages after messageResponse`)
				this.userMessageContent = []
			}

			// CRITICAL FIX: User sent message instead of clicking resume button
			...
			if (responseText || responseImages) {
				console.log(`[Task] User provided additional content with resume button`)
			} else {
				console.log(`[Task] User clicked resume button without additional content`)
			}
		}
		...
		if (responseText) {
			console.log(`[Task] Adding user message to newUserContent: "${responseText.slice(0, 50)}..."`)
			newUserContent.push({
				type: "text",
		...
		if (responseImages && responseImages.length > 0) {
			console.log(`[Task] Adding ${responseImages.length} images to newUserContent`)
			newUserContent.push(...formatResponse.imageBlocks(responseImages))
		}

		console.log(`[Task] Final newUserContent length: ${newUserContent.length}, starting task loop`)
		...
		while (!this.abort) {
			...
			if (didEndLoop) {
				...
				if (allowNoToolsInChatMode && currentMode === "chat") {
					...
					// CRITICAL FIX: Process both direct messages and queued messages in chat mode
					// First, check if there are queued messages to process
					if (!this.messageQueueService.isEmpty()) {
						console.log("[Task] Chat mode: Processing queued messages")
						this.processQueuedMessages()
					}

					// Then process direct user messages
					nextUserContent = [...this.userMessageContent]
					this.userMessageContent = [] // Clear after consuming
					console.log(`[Task] Chat mode: Processing ${nextUserContent.length} direct user messages`)
					// Continue the loop to process the messages
				} else {
		...
		try {
			if (!this.messageQueueService.isEmpty()) {
				...
				if (queued) {
					...
					this.say("user_feedback", queued.text, queued.images).catch((error) => {
						console.error("Failed to display queued user message:", error)
					})

					// Then, add the message content to userMessageContent for processing
					if (queued.text && queued.text.trim()) {
						this.userMessageContent.push({ type: "text", text: queued.text.trim() })
					}
