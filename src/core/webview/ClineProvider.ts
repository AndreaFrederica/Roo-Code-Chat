...
import { getRegexProcessorManager } from "../processors/RegexProcessorManager"
	...
	implements vscode.WebviewViewProvider, TelemetryPropertiesProvider, TaskProviderLike, WebSocketMessageHandler
{
	...
	constructor(
		...
		// Initialize regex processor manager
		this.initializeRegexProcessor().catch((error) => {
			this.log(`Failed to initialize regex processor: ${error}`)
		})

		// Initialize Roo CHAT Cloud profile sync.
	...
	/**
	 * Initialize regex processor based on experimental settings
	 */
	private parseTsProfileKey(profileKey: string): { profileName: string; scope?: "global" | "workspace" } {
		const parts = profileKey.split("|:|")
		if (parts.length === 2) {
			const scope = parts[1]
			if (scope === "global" || scope === "workspace") {
				return { profileName: parts[0], scope }
			}
		}

		return { profileName: parts[0] ?? profileKey }
	}

	private async resolveCurrentTsProfile(enabledProfiles?: string[]): Promise<CurrentTsProfile | null> {
		const profileKeys =
			enabledProfiles && enabledProfiles.length > 0
				? enabledProfiles.filter(Boolean)
				: ((this.contextProxy.getValue("enabledTSProfiles") as string[]) || []).filter(Boolean)

		if (profileKeys.length === 0) {
			this.cachedTsProfileKey = null
			this.cachedTsProfile = null
			return null
		}

		try {
			const extendedService = await getExtendedTSProfileService(this.context)
			const profiles = await extendedService.loadAllTsProfiles()

			for (const key of profileKeys) {
				if (this.cachedTsProfileKey === key && this.cachedTsProfile) {
					return this.cachedTsProfile
				}

				const { profileName, scope } = this.parseTsProfileKey(key)
				const match = profiles.find((profile) => {
					const profileBaseName = path.basename(profile.path, path.extname(profile.path))
					const matchesName = profile.name === profileName || profileBaseName === profileName

					if (scope) {
						return matchesName && profile.scope === scope
					}
				...
				const resolved: CurrentTsProfile = {
					profileName: match.name,
					profilePath: match.path,
					scope: (match.scope ?? "workspace") as "global" | "workspace",
					mixinPath: match.mixinPath,
				}

				this.cachedTsProfileKey = key
				this.cachedTsProfile = resolved
				return resolved
			}
		} catch (error) {
			this.log(
				`[RegexProcessor] Failed to resolve current TSProfile: ${
					error instanceof Error ? error.message : String(error)
				}`,
			)
		}

		this.log(`[RegexProcessor] Unable to match enabled TSProfile keys: ${profileKeys.join(", ")}`)
		this.cachedTsProfileKey = null
		this.cachedTsProfile = null
		return null
	}

	private async initializeRegexProcessor(): Promise<void> {
		try {
			const state = await this.getState()
			const experiments = state.experiments ?? experimentDefault

			// Check if ST regex processor is enabled
			const isRegexProcessorEnabled = experiments.stRegexProcessor ?? false

			this.regexProcessorManager.setEnabled(isRegexProcessorEnabled)

			if (isRegexProcessorEnabled) {
				this.log("ST Regex Processor is enabled")

				// Initialize processors if there's a resolved TSProfile
				const currentTsProfile = await this.resolveCurrentTsProfile(state.enabledTSProfiles)
				if (currentTsProfile) {
					await this.reloadRegexProcessors(currentTsProfile)
				} else {
					this.log("No TSProfile resolved for regex processor initialization")
				}
			} else {
				this.log("ST Regex Processor is disabled")
			}
		} catch (error) {
			this.log(`Error initializing regex processor: ${error}`)
		}
	}

	/**
	 * Reload regex processors with current profile data
	 */
	async reloadRegexProcessors(currentProfile?: CurrentTsProfile): Promise<void> {
		try {
			if (!this.regexProcessorManager.isProcessorEnabled()) {
				this.log("Regex processor is disabled, skipping reload")
				return
			}
			const state = await this.getState()
			const resolvedProfile = currentProfile ?? (await this.resolveCurrentTsProfile(state.enabledTSProfiles))

			if (!resolvedProfile) {
				this.log("No TSProfile resolved, skipping regex processor initialization")
				return
			}

			// Load profile data
			const profileRaw = await fs.readFile(resolvedProfile.profilePath, "utf-8")
			const parsedProfile = JSON.parse(profileRaw)

			// Load mixin data if available
			let mixinData = null
			let mixinPath = resolvedProfile.mixinPath
			...
			if (mixinPath && (await fileExistsAtPath(mixinPath))) {
				try {
			...
			await this.regexProcessorManager.initializeProcessors(parsedProfile, mixinData)

			this.log(
				`Regex processors reloaded with profile: ${resolvedProfile.profileName} (${resolvedProfile.scope ?? "workspace"})`,
			)
		} catch (error) {
			this.log(`Error reloading regex processors: ${error instanceof Error ? error.message : String(error)}`)
		}
	}

	public setRegexProcessorEnabled(enabled: boolean): void {
		this.regexProcessorManager.setEnabled(enabled)
		if (!enabled) {
			this.cachedTsProfileKey = null
			this.cachedTsProfile = null
		}
	...
	*/
	...
	async getStateToPostToWebview(): Promise<ExtensionState> {
		...
		return {
			...
			currentTsProfile: resolvedCurrentTsProfile
				? {
						profileName: resolvedCurrentTsProfile.profileName,
						profilePath: resolvedCurrentTsProfile.profilePath,
						mixinPath: resolvedCurrentTsProfile.mixinPath,
						scope: resolvedCurrentTsProfile.scope,
					}
				: undefined,
	...
	> {
		...
		const enabledTSProfiles = stateValues.enabledTSProfiles ?? []
		const currentTsProfile = await this.resolveCurrentTsProfile(enabledTSProfiles)
		...
		return {
			...
			useRefactoredSystemPrompt: stateValues.useRefactoredSystemPrompt,
			enableInjectSystemPromptVariables: stateValues.enableInjectSystemPromptVariables,
			...
			tsProfilesHasChanges: false, // Default to false for backend state
			currentTsProfile: currentTsProfile
				? {
						profileName: currentTsProfile.profileName,
						profilePath: currentTsProfile.profilePath,
						mixinPath: currentTsProfile.mixinPath,
						scope: currentTsProfile.scope,
					}
				: undefined,
