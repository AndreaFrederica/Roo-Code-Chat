...
Please help the user with their requests to the best of your abilities.`
	 ...
	 * 生成基于文件的系统提示词
	 */
	async buildFileBasedSystemPrompt(options: {
		context: vscode.ExtensionContext
		cwd: string
		mode: Mode
		customModePrompts?: CustomModePrompts
		...
		if (!context || !cwd) {
			throw new Error("Context and cwd are required to generate a file-based system prompt")
		}

		const segments = PromptSegmentAssembler.createSegments()
		let roleDefinition: string | undefined
		let fileCustomSystemPrompt: string
		...
		try {
			...
			fileCustomSystemPrompt = await loadSystemPromptFile(cwd, mode, variablesForPrompt)
			...
			// 生成角色段
			if (rolePromptData) {
				await this.generateRoleSegments(segments, {
					...options,
					rolePromptData,
					userAvatarRole,
					enableUserAvatar,
					userAvatarVisibility,
					supportsComputerUse: false, // 添加缺失的必需字段
				}, rolePromptData)
			}

			// 生成自定义指令段
			let { roleDefinition: tempRoleDefinition, baseInstructions } = getModeSelection(mode || "act", mode ? customModePrompts?.[mode] : undefined, customModes)
			roleDefinition = tempRoleDefinition
			...
			const customInstructions = await addCustomInstructions(
				...
				{
					language: language ?? formatLanguage(vscode.env.language),
					rooIgnoreInstructions: undefined,
					settings: undefined,
				},
			)
			...
			console.warn("[SystemPromptAssembler] Failed to generate file-based system prompt:", error)
			throw error
		}

		// 组装最终提示词
		const finalPrompt = this.assembleFinalPrompt(segments, {
			...options,
			supportsComputerUse: false, // 添加缺失的必需字段
			segmentOrder: ['roleDefinition', 'customInstructions', 'fileCustomSystemPrompt'],
		})

		// 插入文件内容
		const filePrompt = `${roleDefinition}
...
${fileCustomSystemPrompt}`
		...
		// 创建临时段变量用于处理
		const tempSegments = PromptSegmentAssembler.createSegments()
		PromptSegmentAssembler.setSegment(tempSegments, 'customInstructions', fileCustomSystemPrompt)

		const processedSegments = this.applyRegexProcessing(tempSegments, {
			...options,
			experiments,
			supportsComputerUse: false,
		})
		const processedPrompt = processedSegments.customInstructions || ""

		return filePrompt.replace(fileCustomSystemPrompt, processedPrompt)
	}
