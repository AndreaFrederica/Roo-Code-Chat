 ...
 */
export function applyTemplateVariablesToRole(
	role: Role, 
	userAvatarRole?: Role, 
	enableUserAvatar?: boolean
): Role {
	...
	if (processedRole.description) {
		processedRole.description = replaceTemplateVariables(processedRole.description, charName, userAvatarRole, enableUserAvatar)
	}
	if (processedRole.personality) {
		processedRole.personality = replaceTemplateVariables(processedRole.personality, charName, userAvatarRole, enableUserAvatar)
	}
	if (processedRole.scenario) {
		processedRole.scenario = replaceTemplateVariables(processedRole.scenario, charName, userAvatarRole, enableUserAvatar)
	}
	if (processedRole.first_mes) {
		processedRole.first_mes = replaceTemplateVariables(processedRole.first_mes, charName, userAvatarRole, enableUserAvatar)
	}
	if (processedRole.mes_example) {
		processedRole.mes_example = replaceTemplateVariables(processedRole.mes_example, charName, userAvatarRole, enableUserAvatar)
	}
	if (processedRole.creator_notes) {
		processedRole.creator_notes = replaceTemplateVariables(processedRole.creator_notes, charName, userAvatarRole, enableUserAvatar)
	}
	if (processedRole.system_prompt) {
		processedRole.system_prompt = replaceTemplateVariables(processedRole.system_prompt, charName, userAvatarRole, enableUserAvatar)
	}
	if (processedRole.post_history_instructions) {
		processedRole.post_history_instructions = replaceTemplateVariables(processedRole.post_history_instructions, charName, userAvatarRole, enableUserAvatar)
	}
	...
	if (processedRole.profile) {
		if (processedRole.profile.greeting && typeof processedRole.profile.greeting === 'string') {
			processedRole.profile.greeting = replaceTemplateVariables(processedRole.profile.greeting, charName, userAvatarRole, enableUserAvatar)
		}
		if (processedRole.profile.appearance && typeof processedRole.profile.appearance === 'string') {
			processedRole.profile.appearance = replaceTemplateVariables(processedRole.profile.appearance, charName, userAvatarRole, enableUserAvatar)
		}
		if (processedRole.profile.personality && typeof processedRole.profile.personality === 'string') {
			processedRole.profile.personality = replaceTemplateVariables(processedRole.profile.personality, charName, userAvatarRole, enableUserAvatar)
		}
		if (processedRole.profile.background && typeof processedRole.profile.background === 'string') {
			processedRole.profile.background = replaceTemplateVariables(processedRole.profile.background, charName, userAvatarRole, enableUserAvatar)
		}
		if (processedRole.profile.skills && typeof processedRole.profile.skills === 'string') {
			processedRole.profile.skills = replaceTemplateVariables(processedRole.profile.skills, charName, userAvatarRole, enableUserAvatar)
		}
		if (processedRole.profile.relationships && typeof processedRole.profile.relationships === 'string') {
			processedRole.profile.relationships = replaceTemplateVariables(processedRole.profile.relationships, charName, userAvatarRole, enableUserAvatar)
		}
	}
	...
	if (processedRole.character_book?.entries && Array.isArray(processedRole.character_book.entries)) {
		processedRole.character_book.entries = processedRole.character_book.entries.map(entry => ({
			...entry,
			content: entry.content ? replaceTemplateVariables(entry.content, charName, userAvatarRole, enableUserAvatar) : entry.content,
			comment: entry.comment ? replaceTemplateVariables(entry.comment, charName, userAvatarRole, enableUserAvatar) : entry.comment
		}))
	}
