...
export const addEpisodicMemoryTool: Tool = {
	...
	parameters: {
		properties: {
			xml_memory: {
				type: "string",
				description: "XML格式的记忆数据，包含完整的记忆信息。格式：<memory><content>记忆内容</content><keywords>关键词1,关键词2</keywords><priority>80</priority><is_constant>false</is_constant><emotional_context>开心,紧张</emotional_context><related_topics>工作,生活</related_topics></memory>",
			},
			user_message: {
				type: "string",
				description: "AI显示给用户的友好提示语，用于告知用户记忆已保存。应该是自然的、符合角色性格的中文提示语，例如：'我将这段珍贵的经历保存到了我的记忆中'、'这个美好的瞬间我会永远记住'、'谢谢你与我分享这段经历，我会好好珍藏'等。避免使用技术性或调试性的语言。",
			},
		},
	...
	execute: async (args, _, provider) => {
		// 保持兼容性，内部调用函数式实现
		return await addEpisodicMemoryFunction(provider, { args: args } as any)
	},
}
...
export async function addEpisodicMemoryFunction(
	...
	console.log(`[MemoryTool Debug] addEpisodicMemoryFunction called`)
	...
	// 参数验证
	if (!xml_memory) {
		provider.log(`[MemoryTool] 错误：xml_memory参数为undefined (episodic)`)
		return {
			success: false,
	...
	// 验证 user_message 参数
	if (!user_message) {
		provider.log(`[MemoryTool] 错误：user_message参数为undefined (episodic)`)
		return {
			success: false,
	...
	if (typeof user_message !== 'string') {
		provider.log(`[MemoryTool] 错误：user_message参数类型错误，期望string，实际${typeof user_message} (episodic)`)
		return {
			success: false,
	...
	for (const pattern of invalidPatterns) {
		if (pattern.test(trimmedUserMessage)) {
			provider.log(`[MemoryTool] 警告：user_message包含技术性语言: "${trimmedUserMessage}" (episodic)`)
			// 提供更友好的默认消息
			user_message = generateUserMessageTemplate("episodic")
			break
		}
	}
	...
	if (trimmedUserMessage.length < 2) {
		...
		provider.log(`[MemoryTool] 警告：user_message过长，已截断 (episodic)`)
	}
	...
	try {
		...
		console.log(`[MemoryTool Debug] addEpisodicMemory 解析结果:`, JSON.stringify(memoryData, null, 2))
		...
		const memoryId = await memoryService.addEpisodicMemory(
			...
			memoryData.keywords,
			{
				...
				relatedTopics: memoryData.relatedTopics
			},
			{
				// 传递新增的增强字段
		...
		provider.log(`[MemoryTool] Added episodic memory: ${memoryId.substring(0, 8)}...`)
		...
		provider.log(`[MemoryTool] Failed to add episodic memory: ${errorMessage}`)
		return {
			success: false,
			error: `添加情景记忆失败: ${errorMessage}`
		}
	}
}

// 参数验证schema
export const addEpisodicMemorySchema = z.object({
	content: z.string().min(1, "记忆内容不能为空"),
	keywords: z.array(z.string()).optional(),
	priority: z.number().min(0).max(100).optional(),
	is_constant: z.boolean().optional(),
	emotional_context: z.array(z.string()).optional(),
