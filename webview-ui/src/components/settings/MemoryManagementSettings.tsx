...
export const MemoryManagementSettings: React.FC<MemoryManagementSettingsProps> = ({
  ...
  const handleResponse = (response: MemoryManagementResponse) => {
    ...
    // 重新加载数据
    if (["memoryUpdated", "memoryDeleted", "multipleMemoriesDeleted", "memoriesCleaned", "memoriesImported"].includes(response.type)) {
      loadMemories()
      loadStats()
    }
  }
  ...
  const filteredMemories = useMemo(() => {
    return state.memories.filter(memory => {
      if (state.filter.search && !memory.content.toLowerCase().includes(state.filter.search!.toLowerCase()) &&
          !memory.keywords.some(keyword => keyword.toLowerCase().includes(state.filter.search!.toLowerCase()))) {
        return false
      }

      if (state.filter.memoryType && state.filter.memoryType !== "all" && memory.type !== state.filter.memoryType) {
        return false
      }

      if (state.filter.triggerType && state.filter.triggerType !== "all" && memory.triggerType !== state.filter.triggerType) {
        return false
      }
  ...
  const toggleSelectAll = () => {
    setState(prev => ({
      ...prev,
      selectedMemories: prev.selectedMemories.length === filteredMemories.length
        ? []
    ...
    return (
      <Card
        ...
        className={cn(
          ...
          memory.isConstant && "border-amber-200 bg-amber-50"
        )}
      >
        <CardContent className="p-4">
          <div className="flex items-start justify-between mb-2">
            <div className="flex items-center gap-2">
              ...
              />
              <Badge variant="outline" className={`text-xs bg-${typeColor}-50 text-${typeColor}-700`}>
          ...
          <div className="space-y-4">
            <div>
              ...
              <Textarea
                ...
                onChange={(e) => setState(prev => ({
                  ...prev,
                  editingMemory: prev.editingMemory ? { ...prev.editingMemory, content: e.target.value } : undefined
                }))}
                rows={4}
                placeholder="记忆内容..."
              />
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Select
                  ...
                  onValueChange={(value) => setState(prev => ({
                    ...prev,
                    editingMemory: prev.editingMemory ? { ...prev.editingMemory, type: value as any } : undefined
                  }))}
                >
                  <SelectTrigger>
                    <SelectValue />
              ...
              <div>
                ...
                <Select
                  ...
                  onValueChange={(value) => setState(prev => ({
                    ...prev,
                    editingMemory: prev.editingMemory ? { ...prev.editingMemory, triggerType: value as any } : undefined
                  }))}
                >
                  <SelectTrigger>
                    <SelectValue />
            ...
            <div>
              ...
              <Slider
                ...
                onValueChange={([value]) => setState(prev => ({
                  ...prev,
                  editingMemory: prev.editingMemory ? { ...prev.editingMemory, priority: value } : undefined
                }))}
                className="mt-2"
              />
            </div>
            ...
            <div className="flex items-center space-x-2">
              <Switch
                ...
                onCheckedChange={(checked) => setState(prev => ({
                  ...prev,
                  editingMemory: prev.editingMemory ? { ...prev.editingMemory, isConstant: checked } : undefined
                }))}
              />
  ...
  return (
    <div className="space-y-6">
              ...
              <CardContent>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div className="text-center">
                    <div className="text-2xl font-bold text-blue-600">{state.stats.totalMemories}</div>
                    <div className="text-sm text-gray-500">总记忆数</div>
                  </div>
                  <div className="text-center">
                    <div className="text-2xl font-bold text-green-600">{state.stats.constantMemories}</div>
                ...
                <div className="mt-4 grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <div className="mt-1 space-y-1">
                      {Object.entries(state.stats.triggerTypeDistribution).map(([type, count]) => (
                        <div key={type} className="flex justify-between text-sm">
          ...
          <Card>
            <CardContent className="p-4">
              ...
              <div className="mt-2 flex flex-wrap gap-2 text-xs text-gray-500">
                <span>显示 {filteredMemories.length} / {state.memories.length} 条记忆</span>
                {state.filter.memoryType && state.filter.memoryType !== "all" && (
                  <Badge variant="outline" className="text-xs">
                    类型: {memoryTypeOptions.find(o => o.value === state.filter.memoryType)?.label}
                  </Badge>
                )}
                {state.filter.triggerType && state.filter.triggerType !== "all" && (
                  <Badge variant="outline" className="text-xs">
                    触发: {triggerTypeOptions.find(o => o.value === state.filter.triggerType)?.label}
                  </Badge>
                )}
              </div>
            </CardContent>
          ...
          <Card>
            <CardHeader>
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <Checkbox
                    checked={state.selectedMemories.length === filteredMemories.length && filteredMemories.length > 0}
      ...
      {/* 删除确认对话框 */}
      <AlertDialog open={state.showDeleteDialog} onOpenChange={(open) => setState(prev => ({ ...prev, showDeleteDialog: open }))}>
        <AlertDialogContent>
          <AlertDialogHeader>
        ...
        <DialogContent>
          ...
          <div className="space-y-4">
            ...
            <div>
              <Label>常驻记忆</Label>
              <Select
                value={state.filter.isConstant === undefined ? "all" : state.filter.isConstant.toString()}
                onValueChange={(value) => setState(prev => ({
                  ...prev,
                  filter: {
                    ...prev.filter,
            ...
            <div>
              <Label>优先级范围: {state.filter.priorityRange?.min} - {state.filter.priorityRange?.max}</Label>
              <div className="flex items-center gap-2 mt-2">
                <Slider
                  value={[state.filter.priorityRange?.min || 0]}
                  onValueChange={([min]) => setState(prev => ({
                    ...prev,
                    filter: {
                      ...prev.filter,
              ...
              <div className="flex items-center gap-2 mt-1">
                <Slider
                  ...
                  onValueChange={([max]) => setState(prev => ({
                    ...prev,
                    filter: {
                      ...prev.filter,
                  ...
                  className="flex-1"
                />
                <span className="text-sm w-8 text-center">{state.filter.priorityRange?.max || 100}</span>
              </div>
            </div>
          </div>
