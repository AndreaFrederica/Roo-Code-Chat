// 测试useMarkdownProcessor处理复杂文本
import { useMarkdownProcessor } from './useMarkdownProcessor.ts'
...
function createMockHookContext() {
  const mockState = {
    outputStreamProcessorConfig: {
      builtinRulesEnabled: {
        thinking: true,
        updateVariable: true,
        thinkingEnglish: true,
        thinkingChinese: true,
  ...
  // 模拟useExtensionState
  global.useExtensionState = () => mockState
  
  // 模拟useMixinRules
  global.useMixinRules = () => ({
    regexMixins: {},
    astMixins: {}
  })
}

function testUseMarkdownProcessor() {
  console.log('=== 测试useMarkdownProcessor ===')
...
请选择雾雨魔理沙的下一步行动：`
  ...
  try {
    // 调用useMarkdownProcessor
    const blocks = useMarkdownProcessor(complexText)
    ...
    blocks.forEach((block, index) => {
      ...
      console.log(`  动作: ${block.action || 'none'}`)
      console.log(`  默认折叠: ${block.defaultCollapsed || false}`)
      console.log(`  原始标签: ${block.rawTag || 'none'}`)
      console.log(`  是否完整: ${block.isComplete || false}`)
      
      if (block.children && block.children.length > 0) {
        console.log(`  子节点数: ${block.children.length}`)
      }
    })
    ...
    const variableBlocks = blocks.filter(b => b.type === 'variables' || b.rawTag === 'UpdateVariable')
    
    console.log('\n=== 关键节点检查 ===')
    console.log(`思考块数量: ${thinkingBlocks.length}`)
    console.log(`变量块数量: ${variableBlocks.length}`)
